...
(1..ROOM_ID_MAX).each do |room_id|
  room_sql.push("(#{room_id}, 'ひたすら椅子を描く部屋【#{room_id}】', '2016-09-22 00:00:00.#{sprintf('%06d', rand(1000000))}' + INTERVAL #{room_id} SECOND, 1028, 768)")
  ...
  strokes.each do |stroke|
    ...
    stroke_sql.push("(#{stroke_id}, #{room_id}, '2016-09-22 00:00:00.#{sprintf('%06d', rand(1000000))}' + INTERVAL #{stroke_id} SECOND, #{s[:width]}, #{s[:red]}, #{s[:green]}, #{s[:blue]}, #{s[:alpha]})")
...
(1..TOKENS_ID_MAX).each do |token_id|
  token_sql.push("(SHA2(CONCAT(RAND(), UUID_SHORT()), 256), '2016-09-22 00:00:00.#{sprintf('%06d', rand(1000000))}' + INTERVAL #{token_id} SECOND)")
  room_id = rand(ROOM_ID_MAX) + 1

  # room/tokenの作成日時よりも前にwatchするのはおかしい
  interval = ((room_id > token_id) ? room_id : token_id) + rand(100) + 1
  date_at = "'2016-09-22 00:00:00.#{sprintf('%06d', rand(1000000))}' + INTERVAL #{interval} SECOND"
