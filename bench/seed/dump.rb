...
# 2. 部屋を作って描く
# 3. mysql -h127.0.0.1 -uisucon -P33060 -Disuketch -p -N -e 'SELECT strokes.id, width, red, green, blue, alpha, x, y FROM strokes JOIN points ON strokes.id = points.stroke_id ORDER BY strokes.id, points.id' | ruby dump.rb > data/foo.json
